...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, man_trans=None):
    cam_infos = []
...
def readColmapCamerasEval(cam_extrinsics, cam_intrinsics, images_folder,  model_path):
    cam_infos = []
...
def readColmapSceneInfoEval(path, images,  model_path):
    ...
    images_dir = os.path.join(path, "images")
    cam_infos_unsorted = readColmapCamerasEval(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=images_dir,  model_path=model_path)
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
