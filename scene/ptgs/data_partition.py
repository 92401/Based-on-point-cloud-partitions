...
class ProgressiveDataPartitioning:
    ...
    def draw_pcd(self, pcd, train_cameras):    #绘制的点云图以及叠加相机的点云图
    ...
    def draw_partition(self, partition_list):  #可视化点云数据的分区边界图
        for partition in partition_list:
            ori_bbox = partition.ori_camera_bbox  #原始边界框
            extend_bbox = partition.extend_camera_bbox  #扩展边界框
        ...
        self.fig.savefig(os.path.join(self.model_path, f'regions.png'),dpi=200)  #regions.png
        return
        
    def run_DataPartition(self, train_cameras):  #整合数据分区步骤
        if not os.path.exists(self.save_partition_data_dir):
            partition_dict = self.Camera_position_based_region_division(train_cameras)  #基于相机位置的划分
            partition_dict, refined_ori_bbox = self.refine_ori_bbox(partition_dict)  #优化后的分区字典和调整后的边界框
            # partition_dict, refined_ori_bbox = self.refine_ori_bbox_average(partition_dict) #根据中点作为边界
            partition_list = self.Position_based_data_selection(partition_dict, refined_ori_bbox)  #从分区中选择点云数据
            self.draw_partition(partition_list)  #分区可视化
            self.partition_scene = self.Visibility_based_camera_selection(partition_list)  # 输出经过可见性筛选后的场景 包括相机和点云
            self.save_partition_data()
        else:
            self.partition_scene = self.load_partition_data()  #直接加载分区数据
    ...
    def refine_ori_bbox_average(self, partition_dict):  #每个分区的初始信息
        ...
        # 2.按照先x轴对相机的边界进行修正，以相邻两个分区边界坐标的平均值作为公共边界   合并x方向的线
        for m in range(1, self.m_region+1):
            for n in range(1, self.n_region+1):
                ...
                mid_z = (max_z_1 + min_z_2) / 2  #公共边界
                bbox_with_id[partition_idx_1] = [min_x_1, max_x_1, min_z_1, mid_z]  #分出两快的边界
        ...
        for m in range(1, self.m_region + 1):  #m_region只是一个数字
            if m + 1 == self.m_region + 1:
                # 如果是最后一块就退出
                break
            max_x_left = -np.inf  #负无穷
            min_x_right = np.inf
            ...
            for n in range(1, self.n_region + 1):  # 右边分区
                partition_idx = str(m+1) + '_' + str(n)  #下一行的
                min_x, max_x, min_z, max_z = bbox_with_id[partition_idx]
                mid_x = (max_x_left + min_x_right) / 2
        ...
        new_partition_dict = {f"{partition_id}": cameras["camera_list"] for partition_id, cameras in
                              partition_dict.items()}  #从未缝合的列表中id提取对应的相机列表
        return new_partition_dict, bbox_with_id  #返回新的包含区域id，相机列表的字典
    ...
    def Camera_position_based_region_division(self, train_cameras):
        ...
        for idx, camera in enumerate(train_cameras):
            pose = np.array(camera.camera_center.cpu())  #相机中心坐标
            camera_centers.append(pose)
            CameraPose_list.append(
                CameraPose(camera=camera, pose=pose))  # 世界坐标系下相机的中心坐标

        # 保存相机坐标，用于可视化相机位置
        storePly(os.path.join(self.partition_dir, 'camera_centers.ply'), np.array(camera_centers), np.zeros_like(np.array(camera_centers)))  #相机坐标保存成ply文件

        # 2.沿着x轴将相机分成m部分
        m_partition_dict = {}
        total_camera = len(CameraPose_list)  # 获取相机总数
        num_of_camera_per_m_partition = total_camera // m  # m个部分，每部分相机数量
        sorted_CameraPose_by_x_list = sorted(CameraPose_list, key=lambda x: x.pose[0])  # 相机列表按照x轴坐标排序
        ...
        for i in range(m):  # 按照x轴将所有相机分成m部分
            m_partition_dict[str(i + 1)] = {"camera_list": sorted_CameraPose_by_x_list[
                                           i * num_of_camera_per_m_partition:(i + 1) * num_of_camera_per_m_partition]}   #取出第i个区域均分到的相机
            if i != m-1:
                ...
                m_partition_dict[str(i + 1)].update({"x_mid_camera": None})  # 最后一块不需要mid_camera 当前分区的最后一个相机
    ...
    def get_point_range(self, points):  #获取输入点云中 X、Y 和 Z 方向的边界范围
        """获取当前点云的x y z边界"""
        x_list = points[:, 0]
        y_list = points[:, 1]
        z_list = points[:, 2]
    ...
    def Position_based_data_selection(self, partition_dict, refined_ori_bbox):
        ...
        point_num = 0
        point_extend_num = 0
        for partition_idx, camera_list in partition_dict.items():   #遍历每个分区及对应的相机列表
            min_x, max_x, min_z, max_z = refined_ori_bbox[partition_idx]
            ori_camera_bbox = [min_x, max_x, min_z, max_z]  #原始的边界框，
            extend_camera_bbox = [min_x - self.extend_rate * (max_x - min_x),
                                  ...
                                  max_z + self.extend_rate * (max_z - min_z)]  #扩展之后的边界框
            ...
            for camera_pose in camera_list:
                ori_camera_centers.append(camera_pose.pose)   #当前分区的所有相机位置保存到 ori_camera_centers 列表中
    ...
    def point_in_image(self, camera, points):
        """使用投影矩阵将角点投影到二维平面"""
        # 获取点在图像平面的坐标   将世界坐标系中的点转换到摄像机坐标系。
        R = camera.R
        T = camera.T
        w2c = np.eye(4)
        w2c[:3, :3] = np.transpose(R)
        w2c[:3, 3] = T
        fx = camera.image_width / (2 * math.tan(camera.FoVx / 2))  #计算xy方向的焦距
        ...
        points_camera = points_camera[np.where(points_camera[:, 2] > 0)]  # [n, 3]  这里需要根据z轴过滤一下点，保留摄像机坐标系中 z 值大于 0 的点。
        points_image = np.dot(intrinsic_matrix, points_camera.T)  # [3, n]  #将摄像机坐标系下的 3D 点投影到 2D 图像平面
        ...
        return points_image, points_image[mask], mask  #投影后的2D点坐标，图像范围内有效的2D点 有效点的索引
    ...
    def Visibility_based_camera_selection(self, partition_list):
        ...
        思路：引入空域感知的能见度计算
            ...
            5.将j中所有可以投影到相机s的点云也加入到i中
        :param visible_rate: 能见度阈值 默认为0.25 同论文
        """
        # 复制一份新的变量，用于添加可视相机后的每个部分的所有相机
        # 防止相机和点云被重复添加
        add_visible_camera_partition_list = copy.deepcopy(partition_list)  #对分区列表进行深拷贝
        client = 0
        for idx, partition_i in enumerate(partition_list):  # 第i个partition  ，enumerate自动生成索引
            new_points = []  # 提前创建空的数组 用于保存新增的点
            new_colors = []
            new_normals = []

            pcd_i = partition_i.point_cloud   #第i的区域点云
            partition_id_i = partition_i.partition_id  # 获取当前子区域的编号例如1_2
            # 获取当前partition中点云围成的边界框的8角坐标
            partition_ori_point_bbox = partition_i.ori_point_bbox  #原始边界
            partition_extend_point_bbox = partition_i.extend_point_bbox  #扩展后的边界
            ori_8_corner_points = self.get_8_corner_points(partition_ori_point_bbox)  # 获取原始点云围成的边界的8个角点的坐标
            extent_8_corner_points = self.get_8_corner_points(partition_extend_point_bbox)

            corner_points = []
            for point in extent_8_corner_points.values():
                corner_points.append(point)   #把扩展后的八角点加入变量
            storePly(os.path.join(self.partition_extend_dir, f'{partition_id_i}_corner_points.ply'),
                     np.array(corner_points),
                     np.zeros_like(np.array(corner_points)))  #扩展后的八角点导出ply
            ...
            for partition_j in partition_list:  # 第j个partiiton
                ...
                for cameras_pose in partition_j.cameras:
                    ...
                    for key, point in extent_8_corner_points.items():
                        points_in_image, _, _ = self.point_in_image(camera, np.array([point]))   #将当前点投影到当前相机平面中，points_in_image投影后的2D点坐标
                        if len(points_in_image) == 0: continue
                        proj_8_corner_points[key] = points_in_image[0]   #将分区 i 的八个角点投影到当前相机的图像平面上

                    # 基于覆盖率的点选择
                    # i部分中点云边界框投影在j部分当前图像中的面积与当前图像面积的比值

                      #能投影到的点小于3不能围成一个平面则跳过这个相机
                    pkg = run_graham_scan(list(proj_8_corner_points.values()), camera.image_width, camera.image_height)  #使用凸包算法（run_graham_scan）计算投影区域的面积与图像面积的比值
                    # pkg = run_graham_scan(points_in_image, camera.image_width, camera.image_height)
                    if pkg["intersection_rate"] >= self.visible_rate:
                        collect_names = [camera_pose.camera.image_name for camera_pose in add_visible_camera_partition_list[idx].cameras]  #当前分区i已经存在的image_name
                        ...
                        _, _, mask = self.point_in_image(camera, pcd_j.points)  # 在原始点云上需要新增的点，将在满足可见性的相机将j区域中的该相机可见的点云加入到new_point中
                        ...
                        with open(os.path.join(self.model_path, "graham_scan"), 'a') as f:  #每次相机添加的详细信息记录到日志文件
    ...
    #投影区域面积、图像面积、可见性比率、分区编号、添加的相机 ID，以及本次循环新增的相机数量

            #记录该分区最终新增的相机总数和分区总相机数
