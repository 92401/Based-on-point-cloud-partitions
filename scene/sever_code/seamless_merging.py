...
def extend_inf_x_z_bbox(partition_id, m_region, n_region):
    ...
    x, z = int(partition_id.split("_")[0]), int(partition_id.split("_")[1])  # 获取块所在编号   flag指的是有无边界，分别对应x_min,x_max,z_min,z_max
    if x == 1 and z == 1:  # 左上角
...
def seamless_merge(model_path, partition_point_cloud_dir):
    ...
    for partition in partition_scene:
        ...
        if n > n_region: n_region = n  #获取mn的最大数值
    ...
    for partition in partition_scene:
        ...
        if partition.partition_id == "2_1":   #flag指的是有无边界，分别对应x_min,x_max,z_min,z_max，与extend_inf_x_z_bbox函数配合将在原来大区域的子区域边界扩展到无穷处，
            # 确保无缝合并时不丢失背景元素，同时避免因边界裁剪造成的点云不完整
        ...
        mask = extract_point_cloud(xyz, point_select_bbox)  #使用扩展后的边界裁剪点云
        ...
        fig, ax = plt.subplots()  #裁剪后的点云绘制为2D散点图
        x_pos = xyz[mask][:, 0]  #掩码筛选后的数据
        z_pos = xyz[mask][:, 2]
