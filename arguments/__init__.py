...
class ParamGroup:
    def __init__(self, parser: ArgumentParser, name : str, fill_none = False):
        group = parser.add_argument_group(name)  #创建一个新的参数组
        for key, value in vars(self).items():   #返回实例的属性及其值
            shorthand = False  #下划线开头要生成缩写形式   _example 会生成 --example 和 -e 两种参数
            if key.startswith("_"):
                shorthand = True
                key = key[1:]
            t = type(value)
...
class ModelParams(ParamGroup): 
    def __init__(self, parser, sentinel=False):
        self.sh_degree = 3
        self._source_path = "/data/shenjw/mill-19/resident/test/"  # 包含COLMAP或Synthetic NeRF数据集的源目录的路径。
